...
backendCtrls.controller('OrdersSpecialCtrl', ['$scope', '$routeParams', '$http', '$filter', 'PayType', 'Activity',
  ...
  var setOrders = function (orders) {
    angular.forEach($scope.invoices, function (invoice, key) {
      if (invoice.id == orders.invoice.id) {
        if (invoice.orders[orders.id.toString()]) {
          invoice.orders[orders.id.toString()] = orders;
        } else {
          console.log('該訂單找不到對應發票');
        }
      }
    });
    // var invoice = $scope.invoices[orders.invoice.id];

    // if (!invoice) {
    //   invoice = $scope.invoices[orders.invoice.id] = getInitInvoice(orders);
    // }
